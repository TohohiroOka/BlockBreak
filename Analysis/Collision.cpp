...
//球と球の当たり判定
bool Collision::CheakSphere2Plane(const Sphere& sphere,
...
//レイと平面の当たり判定
bool Collision::CheakRey2Plane(const Ray& ray, const Plane& plane,
	float* distance, DirectX::XMVECTOR* inter) {
	const float epsilon = 1.0e-5f;//誤差吸収用の微小な値

	//面法線とレイの方向ベクトルの内積
	float d1 = XMVector2Dot(plane.normal, ray.dir).m128_f32[0];

	//裏面には当たらない
	if (d1 > -epsilon) { return false; }

	//始点と原点の距離（平面の法線方向）
	//面法線とのレイの始点座標（位置ベクトル）の内積
...
//レイと法線付き三角形の当たり判定
bool Collision::CheakRey2Triangle(const Ray& ray, const Triangle& triangle,
	float* distance, DirectX::XMVECTOR* inter) {
	Plane plane;
	XMVECTOR interPlane;
	...
	//レイと平面が当たっていないなら
	if (!CheakRey2Plane(ray, plane, distance, &interPlane)) { return false; }
...
//レイと球の当たり判定
bool Collision::CheakRey2Sphere(const Ray& ray, const Sphere& sphere,
	float* distance, DirectX::XMVECTOR* inter) {
	...
	//tが負である場合、レイは球の内側から開始しているので1をゼロにクランプ
	if (t < 0) { t = 0.0f; }
	if (distance) { *distance = t; }
